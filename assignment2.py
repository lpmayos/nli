...
def draw_graph_with_labels(graph):
    ...
    # graph drawing
    nx.draw_networkx_nodes(graph, pos, node_size=node_size, alpha=node_alpha, node_color=node_color)
    nx.draw_networkx_edges(graph, pos, width=edge_tickness, alpha=edge_alpha, edge_color=edge_color)

    # draw node labels
    nx.draw_networkx_labels(graph, pos, font_size=node_text_size, font_family=text_font)

    # draw edge labels
    edge_labels = {}
    for edge in graph.edges():
        edge_labels[edge] = graph.edge[edge[0]][edge[1]]['text']
    nx.draw_networkx_edge_labels(graph, pos, edge_labels=edge_labels, label_pos=edge_text_pos)
...
def parse_answer(graph, node, answer):
    ...
    edges = [a for a in graph.edges() if a[0] == node]
    
    # get first edge that contains any of the words on 'answer'
    for edge in edges:
        text = graph.edge[edge[0]][edge[1]]['text']
...
def create_dialog(graph):
    ...
    while graph.successors(node):
        answer = raw_input(graph.node[node]['text'] + '\n>> ')
        edge = parse_answer(graph, node, answer)
        node = edge
    print graph.node['goodbye']['text']
    return
